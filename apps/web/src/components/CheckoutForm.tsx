...
export function CheckoutForm() {
  ...
  useEffect(() => {
    ...
    api
      .quote(toQuoteLines(lines), fulfilment)
      .then(setQuote)
      .catch((e) =>
        setError(
          e instanceof ApiError ? e.message : "Could not price this order.",
        ),
      );
  }, [lines, fulfilment]);
  ...
  if (lines.length === 0) {
    return (
      <div className="max-w-md mx-auto text-center py-24 px-6">
        ...
        <Link href="/menu" className="btn-primary mt-4 px-6">
          Browse the menu
        </Link>
      </div>
    );
  ...
  const deliveryReady =
    fulfilment === "collection" || (form.line1.trim() && form.postcode.trim());
  const canSubmit =
    Boolean(quote?.complete) &&
    Boolean(quote?.storeOpen) &&
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8 lg:grid lg:grid-cols-[1fr,400px] lg:gap-10 lg:items-start">
      <div>
        <h1 className="text-2xl font-medium mb-6">Checkout</h1>

        <section aria-labelledby="how" className="mb-8">
          <h2 id="how" className="text-sm font-medium mb-3">
            How would you like it?
          </h2>
          <div className="grid grid-cols-2 gap-3">
            <Choice
              on={fulfilment === "collection"}
              onClick={() => setFulfilment("collection")}
              title="Collection"
              sub={
                quote
                  ? `Ready in about ${quote.prepTimeMinutes} minutes · free`
                  : "Free"
              }
            />
            <Choice
              ...
              sub={
                quote?.deliveryFee
                  ? `${money(quote.deliveryFee)} · within 2 miles`
                  : "Within 2 miles"
              }
            />
          </div>
        ...
        {fulfilment === "delivery" && (
          <section aria-labelledby="addr" className="mb-8">
            <h2 id="addr" className="text-sm font-medium mb-3">
              Deliver to
            </h2>
            <div className="grid sm:grid-cols-2 gap-3">
              ...
              />
              <Field
                label="Flat, building (optional)"
                value={form.line2}
                onChange={set("line2")}
              />
              <Field
                label="Buzzer or gate code (optional)"
                value={form.addressNotes}
                onChange={set("addressNotes")}
              />
            </div>
          </section>
        )}
        ...
        <section aria-labelledby="you" className="mb-8">
          <h2 id="you" className="text-sm font-medium mb-3">
            Your details
          </h2>
          <div className="grid sm:grid-cols-2 gap-3">
            <Field
            ...
            />
            <Field
              label="Email for the receipt (optional)"
              value={form.email}
              onChange={set("email")}
              type="email"
        ...
        <section aria-labelledby="pay" className="mb-8">
          <h2 id="pay" className="text-sm font-medium mb-3">
            Payment
          </h2>
          <div className="grid grid-cols-2 gap-3">
            <Choice
              on={payment === "card"}
              onClick={() => cardsAvailable && setPayment("card")}
              disabled={!cardsAvailable}
              title="Card"
              sub={
                cardsAvailable ? "Handled by Stripe" : "Not available right now"
              }
            />
            <Choice
              on={payment === "on_collection"}
              onClick={() =>
                fulfilment === "collection" && setPayment("on_collection")
              }
          ...
          {payment === "card" && (
            <p className="text-xs text-ink-muted mt-3">
              You will be taken to Stripe to pay, then brought back to track
              your order. In test mode, card 4242 4242 4242 4242 with any future
              date works.
            </p>
          )}
          {fulfilment === "delivery" && !cardsAvailable && (
            <p role="alert" className="text-xs text-bad mt-3">
              Delivery needs card payment, which is unavailable at the moment.
      ...
      <aside className="card p-5 lg:sticky lg:top-24">
        ...
        {(error || problems.length > 0) && (
          <div
            role="alert"
            className="mt-4 rounded-lg bg-bad-bg text-bad p-3 text-sm"
          >
            {error && <p>{error}</p>}
            {problems.length > 0 && (
...
function Choice({
  ...
  return (
    ...
    >
      <span className={`block text-sm ${on ? "font-medium" : "text-ink-soft"}`}>
        {title}
      </span>
