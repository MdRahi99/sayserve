    ...
    <>
      ...
      <section className="px-4 lg:px-8 pt-8 lg:pt-20 pb-12 lg:pb-24 max-w-6xl mx-auto">
        <div className="lg:grid lg:grid-cols-2 lg:gap-16 lg:items-center">
          <div>
            <p className="tag bg-ok-bg text-ok">Open until 23:00 · collection 15 min</p>

            <h1 className="text-4xl lg:text-6xl font-medium tracking-tight mt-4 leading-[1.05]">
              Say it.<br />We serve it.
            </h1>
      ...
      <section className="border-t border-line bg-card">
        <div className="px-4 lg:px-8 py-14 lg:py-20 max-w-6xl mx-auto">
          <h2 className="text-2xl lg:text-3xl font-medium">Three ways to order. One cart.</h2>
          <p className="text-ink-soft mt-3 max-w-xl">
            Whichever you use, the same rules apply: prices come from the menu, and
            anything the assistant adds you can still change by hand.
          </p>
      ...
      <section className="px-4 lg:px-8 py-14 lg:py-20 max-w-6xl mx-auto">
        ...
        <ol className="grid sm:grid-cols-2 lg:grid-cols-4 gap-8 mt-10">
          <Step n="1" title="Order" body="Tap, type or talk. Add what you like and change your mind as often as you want." />
          <Step n="2" title="Checkout" body="Collection or delivery, card or pay at the counter. No account needed." />
          <Step n="3" title="Watch" body="The kitchen accepts it and your page moves on its own. No refreshing." />
          <Step n="4" title="Collect" body="Come in when it says ready. Order the same again next time in one tap." />
        </ol>
      </section>
      ...
      <section className="border-t border-line bg-card">
        <div className="px-4 lg:px-8 py-14 lg:py-20 max-w-6xl mx-auto">
          <div className="flex items-baseline justify-between gap-4">
            <h2 className="text-2xl lg:text-3xl font-medium">Popular right now</h2>
            <Link href="/menu" className="text-sm text-accent hover:underline">See the whole menu</Link>
          </div>
          <div className="mt-8">
            <PopularItems />
          </div>
      ...
      <section className="px-4 lg:px-8 py-14 lg:py-20 max-w-6xl mx-auto">
        <div className="bg-surface rounded-2xl p-8 lg:p-12">
          <h2 className="text-2xl lg:text-3xl font-medium">Have a look at both sides</h2>
          <p className="text-ink-soft mt-3 max-w-lg">
            One press puts you in as a customer, or behind the counter on the kitchen
            board with orders already coming in.
          </p>
          <DemoBanner bare />
        </div>
      </section>
      ...
      <section className="border-t border-line bg-card">
        <div className="px-4 lg:px-8 py-14 lg:py-20 max-w-6xl mx-auto">
          <p className="text-xs tracking-wide uppercase text-ink-muted">About this project</p>
          ...
          <p className="text-ink-soft mt-4 max-w-2xl">
            ...
            decades. SayServe reads that directly, in about ten milliseconds, for nothing.
            The model is the fallback for the messy ones, and even then it only proposes a
            cart: the server prices it, checks it against the live menu, and owns the order.
          </p>

          <dl className="grid grid-cols-2 lg:grid-cols-4 gap-8 mt-10">
            <Stat value="100%" label="of test orders finished without a model call" />
          ...
          <p className="text-sm text-ink-soft mt-10 max-w-2xl">
            Built with Next.js, Node, Express, MongoDB, Socket.io and Stripe. The pricing
            engine, the order state machine and the assistant are all tested on every push.
          </p>

          <div className="flex flex-wrap gap-3 mt-8">
            <a href={GITHUB} target="_blank" rel="noreferrer" className="btn-ghost px-5">
              Code on GitHub
            </a>
            <a href={PORTFOLIO} target="_blank" rel="noreferrer" className="btn-ghost px-5">
              More of my work
            </a>
          </div>
        </div>
...
function Step({ n, title, body }: { n: string; title: string; body: string }) {
  return (
    <li>
      <span className="w-8 h-8 rounded-full bg-ink text-white grid place-items-center text-sm">{n}</span>
