 ...
 */
/**
 * Rebuilt every few minutes rather than on every visit. Next reads this at
 * build time, so it has to be a literal — keep it in step with
  ...
  return (
    <>
      ...
      <section className="bg-warm-glow">
        <div className="px-4 lg:px-8 pt-10 lg:pt-20 pb-14 lg:pb-24 max-w-6xl mx-auto
          ...
          <div className="animate-rise">
            ...
            <h1 className="text-5xl lg:text-7xl font-semibold tracking-tight mt-5 leading-[0.98]">
              Say it.<br /><span className="text-brand">We serve it.</span>
            </h1>
            ...
            <ul className="flex flex-wrap gap-2 mt-8">
              {["Halal certified", "Collection or delivery", "Allergens listed"].map((t) => (
                <li key={t} className="tag bg-card border border-line text-ink-soft">{t}</li>
              ))}
            </ul>
          </div>
      ...
      <section className="px-4 lg:px-8 py-14 lg:py-20 max-w-6xl mx-auto">
        <h2 className="text-3xl lg:text-4xl font-semibold tracking-tight">How it works</h2>
      ...
      <section className="section-tint border-y border-line">
        <div className="px-4 lg:px-8 py-14 lg:py-20 max-w-6xl mx-auto">
          <div className="flex items-baseline justify-between gap-4">
            <h2 className="text-3xl lg:text-4xl font-semibold tracking-tight">Popular right now</h2>
            <Link href="/menu" className="text-sm text-accent hover:underline">See the whole menu</Link>
          </div>
          <div className="mt-8">
            <PopularItems initial={popular} />
          </div>
        </div>
      </section>

      <section className="px-4 lg:px-8 py-14 lg:py-24 max-w-6xl mx-auto">
        <div className="bg-brand-fade rounded-3xl p-8 lg:p-14 text-white shadow-lift">
          <h2 className="text-3xl lg:text-4xl font-semibold tracking-tight">Have a look at both sides</h2>
...
function Feature({ title, body }: { title: string; body: string }) {
  return (
    <div className="card-hover p-6">
      <span className={`tag ${FEATURE_TONE[title] ?? "bg-surface text-ink-soft"} text-xs px-3 py-1`}>
        {title}
      </span>
...
function Step({ n, title, body }: { n: string; title: string; body: string }) {
  return (
    <li className="relative">
      <span className="w-10 h-10 rounded-2xl bg-brand-fade text-white grid place-items-center
                       text-sm font-semibold shadow-soft">{n}</span>
